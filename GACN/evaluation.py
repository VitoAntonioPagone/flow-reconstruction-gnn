...
def run_GCN(input_file, label_file):
    ...
    predicted_velocity_magnitude = velocity_magnitude(
        predicted_features[:, :2] * 7.035423
    )
    ground_truth_velocity_magnitude = velocity_magnitude(
        single_graph.y[:, :2] * 7.035423
    )
    ...
    if hasattr(output_graph, "edge_attr"):
        edge_distance = output_graph.edge_attr.squeeze()
    else:
        print("Edge attributes not found in the output graph.")
        return

    mean_divergence = calculate_mean_divergence_sparse(
        output_graph.edge_index, output_graph.x, edge_distance
    )
    print(f"Mean divergence for the graph: {mean_divergence}")
    rmse_velocity_magnitude_value = rmse_per_component(
        predicted_magnitude_tensor, ground_truth_magnitude_tensor
    )[0]
    mae_velocity_magnitude_value = mae_per_component(
        predicted_magnitude_tensor, ground_truth_magnitude_tensor
    )[0]
    ...
    for i in range(2):
        ...
        rmse_x, rmse_y = rmse_per_component(
            predicted_features[:, :2] * 7.035423, single_graph.y[:, :2] * 7.035423
        )
        mae_x, mae_y = mae_per_component(
            predicted_features[:, :2] * 7.035423, single_graph.y[:, :2] * 7.035423
        )
        ...
        print(
            f"Component {'x-velocity' if i == 0 else 'y-velocity'}: RMSE = {rmse_x if i == 0 else rmse_y}, MAE = {mae_x if i == 0 else mae_y}"
        )
        rmse_values = [
            rmse.item() if isinstance(rmse, torch.Tensor) else rmse
            for rmse in rmse_values
        ]
        mae_values = [
            mae.item() if isinstance(mae, torch.Tensor) else mae for mae in mae_values
        ]

        return (
            rmse_values,
            mae_values,
            rmse_velocity_magnitude_value,
            mae_velocity_magnitude_value,
            mean_divergence,
        )


if __name__ == "__main__":
    stats = {"rmse": defaultdict(list), "mae": defaultdict(list)}

    print(f"Missing Data Percentage: {MISSING_PERCENTAGE}%")
    print(f"Checkpoint Name: {os.path.basename(CHECKPOINT_PATH)}")
    print("---------------------------------------------------")
    input_folder = (
        f"../dataset_graph_full/training_FP/test_input_graphs_{MISSING_PERCENTAGE}"
    )
    ...
    for i, (input_file, label_file) in enumerate(zip(input_files, label_files)):
        if i >= 500:
            break
        rmse_vals, mae_vals, rmse_vel_mag, mae_vel_mag, mean_div = run_GCN(
    ...
    print(
        f"Overall Mean Divergence across the first 50 slices: {overall_mean_divergence:.10f}"
    )
