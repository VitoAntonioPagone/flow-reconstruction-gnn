...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, inputs_dir, labels_dir, subset_ratio=0.2):
        ...
        # List all input files and sort them
        all_input_files = sorted([file for file in os.listdir(self.inputs_dir) if file.endswith('_input.pt')])

        # Calculate the number of files to use based on the subset ratio
