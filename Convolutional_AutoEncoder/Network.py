...
# Example usage
test_input_tensor, _, test_missing_mask_tensor = dataset[0]  # Get a sample input tensor and its single-channel missing mask tensor
...
# Plot the original input tensor and the reconstructed flow tensor
...
for i in range(5):
    ...
    im2 = axes[1, i].imshow(reconstructed_flow_tensor[0, i], cmap='jet', aspect='auto')
    axes[1, i].set_title(f"Reconstructed Flow Tensor - Channel {i + 1}")
