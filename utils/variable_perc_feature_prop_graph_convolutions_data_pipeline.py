...
def process_npz_files(folder):
    npz_files = glob.glob(os.path.join(folder, '*.npz'))
    
    for file_path in npz_files:
        # Randomly choose a percentage between 90 and 99
        percentage = random.randint(90, 99)
        
        print(f"Processing npz file: {file_path} with missing percentage: {percentage}%")
        ...
        output_folder = f'{folder}_variable'  # Updated to use "_variable" instead
        os.makedirs(output_folder, exist_ok=True)
...
if __name__ == "__main__":
    ...
    # Process npz files
    #process_npz_files(TRAIN_OUTPUT_FOLDER)
    process_npz_files(TEST_OUTPUT_FOLDER)
    ...
    create_graphs(TRAIN_OUTPUT_FOLDER + '_variable', NUM_NEIGHBOURS, os.path.join(SAVE_GRAPHS_FOLDER, "train_input_graphs_variable"), is_input=True)
    print("Train input graphs created.")
    sys.stdout.flush()

    create_graphs(VALIDATION_DIR_INPUT + '_variable', NUM_NEIGHBOURS, os.path.join(SAVE_GRAPHS_FOLDER, "validation_input_graphs_variable"), is_input=True)
    ...
    create_graphs(TEST_OUTPUT_FOLDER, NUM_NEIGHBOURS, os.path.join(SAVE_GRAPHS_FOLDER, "test_graphs_variable"), is_input=False)
