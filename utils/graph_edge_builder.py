    ...
    for v in range(len(indices)):
        for j, neighbor in enumerate(indices[v]):
            if neighbor != v:  # remove self-connections
                ...
                edge_index.append([neighbor, v])  # add reverse edge
                edge_attr.append([distances[v, j]])
                edge_attr.append([distances[v, j]])  # add corresponding edge attribute for reverse edge
...
create_graphs(TRAIN_DATA, os.path.join(SAVE_GRAPHS_FOLDER, f"edge_train_graphs_{PERCENTAGE*100:.0f}"), is_input=False)
print("Train graphs created.")
sys.stdout.flush()

create_graphs(TRAIN_INPUTS, os.path.join(SAVE_GRAPHS_FOLDER, f"edge_train_input_graphs_{PERCENTAGE*100:.0f}"), is_input=True)
print("Train input graphs created.")
sys.stdout.flush()

create_graphs(VALIDATION_INPUTS, os.path.join(SAVE_GRAPHS_FOLDER, f"edge_validation_input_graphs_{PERCENTAGE*100:.0f}"), is_input=True)
print("Validation input graphs created.")
sys.stdout.flush()

create_graphs(VALIDATION_DATA, os.path.join(SAVE_GRAPHS_FOLDER, f"edge_validation_graphs_{PERCENTAGE*100:.0f}"), is_input=False)
