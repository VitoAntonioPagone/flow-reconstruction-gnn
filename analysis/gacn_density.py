...
def run_GCN(input_file, label_file):
    ...
    target_velocity_magnitude = (
        calculate_velocity_magnitude(single_graph.y.cpu()) * 7.035423
    )

    print("Prediction complete. Plotting results...")

    fig, ax = plt.subplots(figsize=(6, 6))
    plot_predictions_with_density(
        target_velocity_magnitude.numpy(),
        predicted_velocity_magnitude.numpy(),
        "GAT Predictions",
        ax,
        sample_fraction=1,
    )
